...
public class ApiProjects {
  ...
  public ArrayList<Project> getAllProjectsNames() throws IOException {
    ...
    List<String> listNames = restAssured.get("base").jsonPath().getList("name");
    List<Integer> listId = restAssured.get("base").jsonPath().getList("id");
    List<String> listKind = restAssured.get("base").jsonPath().getList("kind");
